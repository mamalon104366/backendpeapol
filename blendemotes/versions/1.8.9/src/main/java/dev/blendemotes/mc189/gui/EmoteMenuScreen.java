...
public class EmoteMenuScreen extends GuiScreen {
    ...
    public void initGui() {
        ...
        for (int i = 0; i < EmoteConfig.WHEEL_SLOTS; i++) {
            ...
            buttonList.add(new GuiButton(SLOT_BASE + i, x, y, slotW, 20, fontRendererObj.trimStringToWidth(label, slotW - 6)));
        }
        ...
        for (int i = 0; i < perPage; i++) {
            ...
            buttonList.add(new GuiButton(ENTRY_BASE + idx, x, y, entryW, 20, fontRendererObj.trimStringToWidth(label, entryW - 6)));
        }
        int by = height - 28;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(fontRendererObj, I18n.format("blendemotes.menu.title"), width / 2, 8, 0xFFFFFF);
        ClientEmotes client = client();
        ...
        drawCenteredString(fontRendererObj, hint, width / 2, info, 0xAAAAAA);
        int errors = client.library.errors().size();
        if (errors > 0) {
            drawCenteredString(fontRendererObj, I18n.format("blendemotes.menu.errors", errors), width / 2, height - 42, 0xFF6060);
        }
        if (selected != null) {
            ResourceLocation icon = Icons189.get(selected);
            if (icon != null) {
                Draw189.icon(icon, 6, 6, 48);
            }
        }
    }
