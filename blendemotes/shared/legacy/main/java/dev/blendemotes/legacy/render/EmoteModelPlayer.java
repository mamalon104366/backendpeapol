...
public class EmoteModelPlayer extends ModelPlayer {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw,
        ...
        PlayerPose p = LegacyEmotes.client().pose(entity.getUniqueID(), vanilla, slim, ageInTicks / 20.0, pose);
    ...
    public void postRenderArm(float scale, EnumHandSide side) {
        ...
        // vanilla shifts slim arms by half a pixel towards the outside while placing the item
        itemTransform(p, right, slim ? (right ? 0.5 : -0.5) : 0, scale);
    }
    //#else
    @Override
    public void postRenderArm(float scale) {
        ...
        // vanilla shifts slim arms by one pixel while placing the item
        itemTransform(p, true, slim ? 1 : 0, scale);
    }
    //#endif

    private static void itemTransform(PlayerPose p, boolean right, double slimOffset, float scale) {
        Vec3 pivot = p.rig().pivot(right ? PlayerPart.RIGHT_ARM : PlayerPart.LEFT_ARM);
        PlayerPart item = right ? PlayerPart.RIGHT_ITEM : PlayerPart.LEFT_ITEM;
        Mat4 m = p.matrix(item).mul(Mat4.translation(pivot.x + slimOffset, pivot.y, pivot.z));
